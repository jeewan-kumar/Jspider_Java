...
public class Rotate60Left {
    public static void main(String[] args) {
        ...
        int[][] rotatedMatrix = rotate60Left(matrix);

        // Display the rotated matrix
        System.out.println("Matrix after 60-degree Left Rotation:");
    ...
    // Method to rotate a matrix 60 degrees to the left
    private static int[][] rotate60Left(int[][] matrix) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {
                int newRow = i + (int) Math.round(j * Math.tan(Math.toRadians(60)));
                int newColumn = j;
